...
impl ControlSessionBroker {
    ...
    #[cfg_attr(
        ...
        expect(
            dead_code,
            reason = "Output byte inspection is used by the physical adapter follow-up."
        )
    )]
    ...
    fn advance_guest_receive_sequence(&mut self) -> Result<(), BrokerError> {
        if advance_sequence(&mut self.guest_receive_sequence).is_err() {
            return Err(self.fail(BrokerError::SequenceOverflow));
        }
        Ok(())
    }

    fn advance_guest_send_sequence(&mut self) -> Result<(), BrokerError> {
        if advance_sequence(&mut self.guest_send_sequence).is_err() {
            return Err(self.fail(BrokerError::SequenceOverflow));
        }
        Ok(())
    }
    ...
    fn advance_host_receive_sequence(&mut self) -> Result<(), BrokerError> {
        if advance_sequence(&mut self.host_receive_sequence).is_err() {
            return Err(self.fail(BrokerError::SequenceOverflow));
        }
        Ok(())
    }
    ...
    fn advance_host_send_sequence(&mut self) -> Result<(), BrokerError> {
        if advance_sequence(&mut self.host_send_sequence).is_err() {
            return Err(self.fail(BrokerError::SequenceOverflow));
        }
        Ok(())
    }
...
fn advance_sequence(sequence: &mut u64) -> Result<(), BrokerError> {
