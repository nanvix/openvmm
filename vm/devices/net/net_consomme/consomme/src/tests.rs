...
fn static_ipv4_identity_is_exact_and_disables_ipv6_advertisement() {
    ...
    for protocol in [IpProtocol::Tcp, IpProtocol::Udp] {
        assert_eq!(
            consomme
                .state
                .resolve_destination(&"192.168.5.33:8080".parse().unwrap(), protocol),
...
fn gateway_loopback_can_be_denied_with_one_proxy_exception() {
    ...
    for protocol in [IpProtocol::Tcp, IpProtocol::Udp] {
        for port in [8442, 8443, 8444] {
            ...
            assert_eq!(
                consomme.state.resolve_destination(&address, protocol),
                expected,
                "{protocol:?} port {port}"
            );
        }
    ...
    assert_eq!(
        consomme
            .state
            .resolve_destination(&"192.0.2.1:8443".parse().unwrap(), IpProtocol::Udp),
