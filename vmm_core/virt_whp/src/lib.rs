...
#[cfg(guest_arch = "x86_64")]
fn mask_gpa_pinning_enlightenment() -> virt::CpuidLeaf {
        ...
        let cpuid = {
            ...
            if proto_config.hv_config.is_some() {
                if !offload_enlightenments || user_mode_apic {
                    ...
                    // The L0 pinning requirement describes memory owned by
                    // OpenVMM, not memory owned by OpenVMM's guest. OpenVMM
                    // does not expose the GPA pin/unpin hypercalls, so do not
                    // pass this enlightenment through to the guest.
                    cpuid.push(mask_gpa_pinning_enlightenment());
                }
            }
