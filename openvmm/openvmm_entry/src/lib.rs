...
fn microvm_filesystem_from_mount(
    requested: &cli_args::MicrovmMountCli,
...
mod microvm_console_attachment_tests {
    ...
    fn filesystem_denied_paths_are_canonical_and_root_scoped() {
        ...
        let filesystem = effective_microvm_filesystem(
            options.microvm_mount.as_ref(),
            &options.microvm_mount_deny,
            None,
        )
        .unwrap()
        .unwrap();
    ...
    fn filesystem_restore_rejects_missing_or_changed_policy() {
        ...
        assert!(
            effective_microvm_filesystem(
                changed_mode.microvm_mount.as_ref(),
                &[],
                Some(&contract),
            )
            .is_err()
...
async fn vm_config_from_command_line(
    ...
    let effective_microvm_filesystem = if is_microvm {
        effective_microvm_filesystem(
            opt.microvm_mount.as_ref(),
            &opt.microvm_mount_deny,
            restore_machine_contract,
        )?
    } else {
        None
