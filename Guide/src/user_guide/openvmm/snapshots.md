...
- Standard-machine restore still requires matching `--memory` and
  ...
  and rejects overrides. Persisted microVM ABI value 2 and Linux-direct
  boot-layout value 3 are supported; removed boot-layout value 2 snapshots
  require an earlier compatible OpenVMM build.
